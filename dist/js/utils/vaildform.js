...
;(function($){
    ...
    Vaildform.prototype.activeForm = function(form,del_opts){
        ...
        if(del_opts.blur_err_show){
            ...
            form.parent().find("input").removeClass("error");
            //判断有无填写
            if(!_this.operation(form,form,form.data('type'))){
                ...
                parents_dom.append($("<span class='xfd-help-block text-danger'>"+(form.attr('tips'))+"</span>")) : null;
            }
            //判断是否为密码验证
            if(form.attr("recheck")&&form.val().length>0){
                ...
                if(password_value === form.val()){
                    ...
                    password_parents.append($("<span class='xfd-help-block m-b-none text-danger'>您两次输入的密码不一致</span>"))
                }
            }
