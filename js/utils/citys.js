...
(function ($) {
    $.support.cors = true;
    $.citys = function (parameter, getApi) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (options.code) { //如果设置地区编码，则忽略单独设置的信息
                    var c = options.code - options.code % 1e4;//省份
                    if (data[c]) {
                        options.province = c;
                    }
                    c = options.code - (options.code % 1e4 ? options.code % 1e2 : options.code);//城市
                    if (data[c]) {
                        options.city = c;
                    }
                    c = options.code % 1e2 ? options.code : 0;//区
                    console.log(c)
                    if (data[c]) {
                        options.area = c;
                    }
